...
_logger.debug("Cannot import 'milanuncios' API")

class MilanunciosAPI(object):

    def __init__(self, backend):
        """
        :param backend: Milanuncios Backend
        """
...
class MilanunciosCRUDAdapter(AbstractComponent):
    """ External Records Adapter for Milanuncios """

    _name = 'milanuncios.crud.adapter'
    _inherit = ['base.backend.adapter', 'base.milanuncios.connector']
    ...
    def _call(self, method, arguments):
        try:
            milanuncios_api = getattr(self.work, 'milanuncios_api')
        except AttributeError:
            raise AttributeError(
                'You must provide a milanuncios_api attribute with a '
                'MilanunciosAPI instance to be able to use the '
                'Backend Adapter.'
            )
        return milanuncios_api.call(method, arguments)


class GenericAdapter(AbstractComponent):
    _name = 'milanuncios.adapter'
    _inherit = 'milanuncios.crud.adapter'

    _milanuncios_model = None
    _admin_path = None

    def _get_model(self):
        if self._milanuncios_model:
            return self._milanuncios_model
        elif self.model:
